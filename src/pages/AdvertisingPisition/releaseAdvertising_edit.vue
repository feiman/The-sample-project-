      ...
      <div class="peakfire-main">
            <div>
                ...
                <el-form-item label="搜索关键词：" prop="keywordId">
                    <Select style="width:290px" v-model="keywordId" placeholder="" multiple multiple-limit="5" id="select_keyid_edit">
                ...
                <el-form-item label="日期：" prop="startDate">
                    <div>
                        <!-- <el-date-picker style="width:290px"
                            type="daterange" size="small" v-model="startDate" value-format="yyyy-MM-dd"
                            range-separator="-" auto-complete="off"
                ...
                <el-form-item label="出租时间段：" prop="startTime">
                    <div>
                        <el-time-picker is-range style="width:290px" id="seltime"
                            ...
                            auto-complete="off" popper-class="resettime"
                ...
                <el-form-item label="展示图：" prop="showImage">
                    ...
                    <div class="peakfire_titp">
                        提醒：最多上传展示图5张，格式jpg/png,单个图片大小不得超过1M
                    </div>
                </el-form-item></br/>
                ...
                <el-form-item label="内容详情：" style="height:460px;" prop="introduce" >
                        <!-- 加载编辑器的容器 -->
                     <script id="containerses" name="content" type="text/plain" style="min-height:406px;"></script> 
                </el-form-item><br/>
                <el-form-item label=" ">
...
export default {
  ...
  activated() {
    ...
    document.querySelector("#containerses").style.width = `${peakfire_main_W - 200}px`;
    document.querySelector(".text_wid").style.width = `${peakfire_main_W - 200}px`;
    var um = UM.getEditor("containerses");

    var formdata = this.storage.get("rowdata", null, true);
    this.ruleForm = formdata;

    this.keywordId = formdata.keywordId.split(",");
    this.cyclePrice = formdata.cyclePrice / 100;
    this.timePrice = formdata.timePrice / 100;

    var startTime = [];
    // startDate = [],
    // startDate[0] = new Date(formdata.startDate);
    // startDate[1] = new Date(formdata.endDate);

    // this.startDate = startDate;
    startTime[0] = new Date(formdata.startDate + " " + formdata.startTime);
    startTime[1] = new Date(formdata.startDate + " " + formdata.endTime);
    this.startTime = startTime;

    this.AdReleaseid = formdata.id;

    var showImages = formdata.showImage.split(",");
    ...
    setTimeout(() => {
      UM.getEditor("containerses").setContent(formdata.introduce);
    }, 200);
  },
  methods: {
    showDioLog(e){
    ...
    submitForm(formName) {
      ...
      // if (this.startDate) {
      //   this.ruleForm.startDate =moment(this.startDate[0]).format("YYYY-MM-DD");
      //   this.ruleForm.endDate = moment(this.startDate[1]).format("YYYY-MM-DD");
      // }
      if (this.startTime) {
        this.ruleForm.startTime = moment(this.startTime[0]).format("HH:00");
        this.ruleForm.endTime =  moment(this.startTime[1]).format("HH:00");
      }
      // if (this.startDate.length > 0 && this.startTime) {
      //   var asMinutes = moment
      //     .duration(this.startTime[1] - this.startTime[0])
      //     .asMinutes();
      //   var asDays = moment
      //     .duration(this.startDate[1] - this.startDate[0])
      //     .asDays();
      //   this.ruleForm.number = parseInt(asMinutes) / 3 * asDays;
      // }

      this.ruleForm.showImage = String(filelist);
      this.ruleForm.introduce = UM.getEditor("containerses").getContent();
    ...
    addadvertising() {
      ...
      this.ruleForm.id = this.AdReleaseid;
      updatemyAdRelease(this.ruleForm)
        .then(data => {
          if (data.success) {
            this.$message({
              type: "success",
              message: "修改成功，1秒后将返回广告位列表"
            });
            setTimeout(() => {
...
#select_keyid_edit .ivu-select-placeholder{
