            ...
            <template v-for="(item,index) in lister">
                <div class="calendar_item" @mousedown="page_x = index">
                    <div style="width:100%;height:100%;" @click.stop.prevent="addDrag">
                         <!-- 已出租 -->
                        <template v-for="ite in item.leased_out">
                            <div class="calendar_item_leased_out" :class="'_leased_out'+index" :id="'leased_out'+ite.top" :style="{top:ite.top+'px',height:ite.height+'px'}">
                                已出售 ({{ite.top|computed_times}}-{{ite.top+ite.height|computed_times}})
                            </div>
                        </template>
                        <!-- 已发布 -->
                        <template v-for="ite in item.have_released">
                            <div class="calendar_item_have_released" :class="'have_released'+index" :id="'released'+ite.top" :style="{top:ite.top+'px',height:ite.height+'px'}">
                                已发布 ({{ite.top|computed_times}}-{{ite.top+ite.height|computed_times}})
                            </div>
                        </template>
                        <!-- 拖拽层 -->
                    </div>
                    <div>
                        <template v-for="ite in item.dreage">
                            <div class="dreage" :class="'dreage'+index" :style="{height:ite.xheight-ite.xtop+'px'}">
                                <vue-draggable-resizable @dragging="onDrag" :marginTop="ite.xtop" @resizing="onResize" :y="ite.y-ite.xtop" :x="0" :minh="1"
                                class="_released" axis="y" :handles="['bm','tm']" :h='ite.h' :parent="true">
                                    <p class="drag">发布(<span :class="'drag'+index"></span>)</p>
...
export default {
    ...
    data(){
        return {
            leased_out_list:[],
            lister:[{// {xtop:0,xheight:0,y:50,h:30,}
                leased_out:[{top:30,height:30}],
                have_released:[{top:300,height:30}],
                dreage:[]
            },{// {xtop:0,xheight:0,y:50,h:30,}
                leased_out:[{top:30,height:30}],
                have_released:[{top:300,height:30}],
                dreage:[]
            }],
    ...
    methods: {
        ...
        computer_time(start_time='2018-04-11 02:30:00',end_time='2018-04-11 03:00:00 '){
            var startDay = moment(start_time);
            var endDay = moment(end_time);
            var initH = moment.duration(startDay - moment(this.initTime.start)).asMinutes();
