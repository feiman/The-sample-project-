...
export default {
  ...
  computed: {
    // ...mapGetters(['active']),
    day_sum() {
      var day = moment.duration(this.value - new Date()).asWeeks();
      if (day < 0) {
        return "本周";
      }
      return `${parseInt(day) + 1}周后`;
    }
  },
  methods: {
    ...mapMutations({
    ...
    prev() {
      //   不允许 到今天之前的 日期
      if ($("#calendar").fullCalendar("getDate").format('YYYY-MM-DD') == moment().format('YYYY-MM-DD')) {
        return;
      };
      $("#calendar").fullCalendar("prev");
      this.value = $("#calendar").fullCalendar("getDate")
    },
    ...
    curren_timefun(value) {
      return (
        ...
        "至" +
       moment(value.getTime()+86400000*6).format("YYYY-MM-DD")
      );
    }
  },
  watch: {
    value(val) {
      var sval =val
      if (val._d) { sval=val._d;};
      this.curren_time = this.curren_timefun(sval);
      
      this.setSelTime({
        sDate: moment(val).format("YYYY-MM-DD"),
        eDate: moment(sval.getTime()+86400000*6).format("YYYY-MM-DD")
      });
